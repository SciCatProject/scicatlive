...
| env     | `DEV_AI_STATE`        | `<name>`: AI coding assistant (e.g. Claude Code) state directory                                                      | `.claude` | \*                    | Name of the volume used to persist the state of the AI service (Claude Code) across container restarts. It is shared across all DEV-mode node services so that the conversation history/config is preserved and visible from any of them |                         |
